...
class A3C(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def __init__(self, model, optimizer, a2c, t_max, gamma, logger, gpu=None,
                 beta=1e-2, process_idx=0, phi=lambda x: x,
    ...
    def update(self, statevar, action_array):
        ...
        if statevar is None:
            ...
            with state_kept(self.model):
                _, vout = self.model(statevar, action_array)
            R = float(vout.data)
        ...
        if self.process_idx == 0:
            self.logger.info('t2:%s pi_loss:%s v_loss:%s', self.t, pi_loss.data[0], v_loss.data[0][0])
...
#            self.model.unchain_backward()
        self.model.unchain_backward()
        self.model.unchain_backward_all()
    ...
    def act_and_train(self, state, reward, action_array):
        ...
        pout, vout = self.model(statevar, action_array)
        ...
        if self.process_idx == 0:
            self.logger.info('t:%s r:%s a:%s', self.t, reward, action)
        # Update stats
        self.average_value += (
