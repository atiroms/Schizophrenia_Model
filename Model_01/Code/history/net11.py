...
class SiSHead(chainer.Chain):
    def __init__(self, n_actions, input_action, skip_P, n_input_channels = 4, n_output_channels = 512, activation = F.relu, bias = 0.1, batchSize = 1):
        super(SiSHead, self).__init__()
        ...
        self.add_link('FCO5', L.Linear(6272, self.n_output_channels, bias = bias))
#        self.add_link('FCO6', L.Linear(512, self.n_actions))

        self.reset_state()

    def to_cpu(self):
        super(SiSHead, self).to_cpu()
    ...
    def to_gpu(self, device=None):
        super(SiSHead, self).to_gpu(device)
        for nth in range(1, 5):
            if getattr(self, 'E' + str(nth)) is not None:
...
class SiS(chainer.ChainList, recurrent.RecurrentChainMixin):
    def __init__(self, n_actions, input_action, skip_P):
        self.head = SiSHead(n_actions, input_action, skip_P)
        self.pi = policy.FCSoftmaxPolicy(self.head.n_output_channels, n_actions)
        self.v = v_function.FCVFunction(self.head.n_output_channels)
        super(SiS, self).__init__(self.head, self.pi, self.v)

    def to_gpu(self, device):
        super(SiS, self).to_gpu(device=device)
#        self.head.to_gpu(device=device)
#        self.pi.to_gpu(device=device)
#        self.v.to_gpu(device=device)
...
#        self.head.to_cpu()
    ...
    def unchain_backward_all(self):
        self.head.unchain_backward_all()

    def __call__(self, state, action_array):
