...
class A3C(agent.AttributeSavingMixin, agent.AsyncAgent):
    ...
    def __init__(self, model, optimizer, t_max, gamma, gpu=None, beta=1e-2,
        ...
        if gpu is not None and gpu >= 0:
            ...
            self.shared_model.to_gpu(device=gpu)
...
#        async.assert_params_not_shared(self.shared_model, self.model)
    ...
    def update(self, statevar, action_array):
        ...
        if statevar is None:
            ...
            with state_kept(self.model):
                _, vout = self.model.pi_and_v(statevar, action_array)
            R = float(vout.data)
        ...
        if self.process_idx == 0:
            logger.debug('pi_loss:%s v_loss:%s', pi_loss.data[0], v_loss.data[0][0])
...
#            self.model.unchain_backward()
        self.model.unchain_backward()
        self.model.head.ConvLSTM1.h.unchain_backward()
        self.model.head.ConvLSTM2.h.unchain_backward()
        self.model.head.ConvLSTM3.h.unchain_backward()
        self.model.head.ConvLSTM4.h.unchain_backward()
    ...
    def act_and_train(self, state, reward, action_array):
        ...
        if self.t - self.t_start == self.t_max:
            self.update(statevar, action_array)

        self.past_states[self.t] = statevar
        pout, vout = self.model.pi_and_v(statevar, action_array)
        #pout as SoftMaxDistribution class
        action = pout.sample()
        #samples a batch of actions from a batch of action probabilities
        ...
        if self.process_idx == 0:
            logger.debug('t:%s r:%s a:%s', self.t, reward, action)
        # Update stats
        self.average_value += (
    ...
    def stop_episode_and_train(self, state, reward, action_array, done=False):
        self.past_rewards[self.t - 1] = reward
        if done:
