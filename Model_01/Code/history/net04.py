...
class SiS(chainer.Chain):
    ...
    def __call__(self, x, activation = F.relu, action_array = None, test = True):

        if self.E1 is None:
            self.E1 = variable.Variable(self.xp.zeros((self.batchSize, 8, 84, 84), dtype=np.float32), volatile = 'auto')
        if self.E2 is None:
            self.E2 = variable.Variable(self.xp.zeros((self.batchSize, 64, 21, 21), dtype=np.float32), volatile = 'auto')
        if self.E3 is None:
            self.E3 = variable.Variable(self.xp.zeros((self.batchSize, 128, 7, 7), dtype=np.float32), volatile = 'auto')
        if self.E4 is None:
            self.E4 = variable.Variable(self.xp.zeros((self.batchSize, 128, 7, 7), dtype=np.float32), volatile = 'auto')
        I6 = self.FCI6(action_array)
        I5 = self.xp.reshape(self.FCI5(I6), (self.batchSize, 128, 7, 7))
        ...
        A2 = F.relu(self.ConvA2(self.E1))
        self.E2 = F.concat((F.relu(A2 - P2), F.relu(P2 - A2)))
        A3 = F.relu(self.ConvA3(self.E2))
        self.E3 = F.concat((F.relu(A3 - P3), F.relu(P3 - A3)))
        A4 = F.relu(self.ConvA4(self.E3))
        self.E4 = F.concat((F.relu(A4 - P4), F.relu(P4 - A4)))
        O5 = self.FCO5(self.xp.reshape(self.E4, (1, 6272)))
        O6 = self.FCO6(O5)
        return O6
